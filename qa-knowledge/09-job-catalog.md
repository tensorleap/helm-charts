...
- **Trigger:** auto-runs when the dashlet mounts. `POST /visualizations/populationExploration` then polls `POST /visualizations/getPopulationExplorationStatus` every ~3s. **Blocked while an Evaluate/Update-Evaluate is in progress** (UserError before the job is created).
- **The digest** is minted server-side in one place (`calcPopulationExplorationDigest`) from the population params + `insightsRevision` + `sample_visualizers_revision` + seed count + teamId; the client-passed digest is ignored. A new Evaluate bumps revisions → new digest → new scatter path → UI re-runs. (Status `NOT_FOUND` after a new eval is *expected*, not a bug.)
