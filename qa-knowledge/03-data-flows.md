...
> Recall from [README.md](README.md): node-server publishes the job-creation
> request to RabbitMQ (`engine-job-creation`) and the **orchestrator** creates
> the k8s Job via the k8s API; "Evaluate" is `type:TRAINING / subType:Evaluate`;
> the **main engine pod** creates the runtime resources.
...
| 4 | node-server (`engine.ts`) | Uploads request JSON to bucket; reads `engine-job-template-cm`; resolves machine type + auto-settings; uploads `k8s_engine_generic_settings.json` + `k8s_engine_redis_settings.json`; **publishes a create request to RabbitMQ queue `engine-job-creation`** (9-min TTL); the **orchestrator** admits it against a cluster-memory ledger and **creates the k8s Job**; job stays `UNSTARTED` until the orchestrator's `active_jobs_report` promotes it to `PENDING` | `kubectl -n tensorleap get jobs` shows **`evaluate-<jobId>`** with labels `jobId=<jobId>`, `projectId=<projectId>`, `hasWorker=true`; bucket has the payload JSON + `pods-settings/<versionId>/…`; node-server log `Engine job creation request published`; UI gets a PENDING notification |
...
| 9 | main engine pod (`evaluate_epoch`) | Pops ready samples, runs **inference**, computes metrics inline (default), pushes batches round-robin across **sharded** keys `streaming_evaluate_<jobId>_s<N>_queue` (default 8 shards, `STREAMING_QUEUE_SHARDS`; or `metrics_<jobId>` in dedicated mode) | pod log `Pushing  elements to redis key: streaming_evaluate_<jobId>_s<N>_queue`; `LLEN` on the shard keys > 0; backpressure = push loop blocks when a shard's len > per-shard threshold |
| 10 | streaming-handler | Pulls bulks (≤1000 — `STREAMING_BULK_SIZE` — or 20s), uploads latent space to bucket (evaluate queues only), indexes metrics+metadata to ES (`es_metrics_index`) | `kubectl logs deploy/streaming-handler-<jobId>` `indexing docs to elasticsearch`; `GET <es_metrics_index>/_count` rises; bucket latent-space objects appear under `inference_artifact_id` |
...
- Redis backpressure → `LLEN` on the `streaming_evaluate_<jobId>_s<N>_queue` shards stays high; `noeviction`
...
| 3 | web-ui → node-server | `getCollectionSampleOrder` returns **cursor-paginated** sample identities (client `PAGE_SIZE=10_000`, matching the server's `MAX_PAGE_SIZE`) with server-side **sort/filter**; `loadMore` rides `search_after` cursors | POST `…/sample-collection/getCollectionSampleOrder` 200 `{rows/samples, total, cursor}`; `total` drives the scrollbar |
