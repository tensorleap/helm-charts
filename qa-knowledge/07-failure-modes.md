...
| User signed out unexpectedly while active (no 401 seen) | **not** a failure — was possible pre-fix via a transient token-refresh rejection or a `checkLoginIframe` timeout; both now tolerated: `onTokenExpired`/api-client refresh failures only warn-and-retry, and `checkLoginIframe` is disabled, so only a genuine `onAuthLogout` (refresh token truly dead) ends the session | `web-ui/src/auth/AuthProvider.tsx`, `web-ui/src/core/api-client.tsx` — no `onAuthRefreshError` handler exists; console shows `Token refresh failed, retrying on next request` / `sending request anyway` instead of a forced logout |
